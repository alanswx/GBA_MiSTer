...
/*
	thumbdecoder_ver thumbdecoder_ver_inst
	(
		.CLK(CLK) ,												// input  CLK
		.nRESET(nRESET) ,										// input  nRESET
		.CLKEN(CLKEN) ,										// input  CLKEN
		.InstForDecode(IPDR_ToThumbDecoder) ,			// input [31:0] InstForDecode
		
		.ExpandedInst(ExpandedInst_ver) ,				// output [31:0] ExpandedInst
		
		.HalfWordAddress(IPDR_HalfWordAddress) ,		// input  HalfWordAddress
		.ThumbDecoderEn(ThDC_ThumbDecoderEn) ,			// input  ThumbDecoderEn
		.StagnatePipeline(IPDR_StagnatePipeline) ,	// input  StagnatePipeline
		
		.ThBLFP(ThBLFP_ver) ,								// output  ThBLFP
		.ThBLSP(ThBLSP_ver) ,								// output  ThBLSP
		.ThADR(ThADR_ver) 									// output  ThADR
	);
	
(*keep*) wire [31:0] ExpandedInst_ver;
(*keep*) wire ThBLFP_ver;
(*keep*) wire ThBLSP_ver;
(*keep*) wire ThADR_ver;
*/
